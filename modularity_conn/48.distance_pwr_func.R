...
exp.trace1 <- exp(-d1$mu * (0:(max(dat1)+1)))
...
exp.trace2 <- exp(-d2$mu * (0:(max(dat2)+1)))
...
exp.trace3 <- exp(-d3$mu * (0:(max(dat3)+1)))
...
exp.trace4 <- exp(-d4$mu * (0:(max(dat4)+1)))
...
plot(log10(cutoff:(max(dat1) + 1)), log10(exp.trace1)[-1], lwd = 3, col = thepalOrder[1], type = "l")
lines(log10(cutoff:(max(dat2) + 1)), log10(exp.trace2)[-1], lwd = 3, col = thepalOrder[2])
lines(log10(cutoff:(max(dat3) + 1)), log10(exp.trace3)[-1], lwd = 3, col = thepalOrder[3])
lines(log10(cutoff:(max(dat4) + 1)), log10(exp.trace4)[-1], lwd = 3, col = thepalOrder[4])
legend("bottomleft",legend=c("Highly ordered","Some order", "Almost random", "Random"),pch=15,col=c(thepal[1:4]))
