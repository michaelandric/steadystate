...
for (n in c("gamma", "rate"))
{
    ...
    print(summary(aov(rank(dist_df[,1]) ~ condition + Error(subject/condition), data=dist_df)))   # rank transform RV more powerful
    #print(anova(lme(fixed = gamma ~ condition, random = ~-1|subject, data = dist_df)))   # another way to express above line
