...
for (i in 1:length(subjects))
{
    ...
    subj_mat[,i] <- as.matrix(read.table(paste("preserved_",subjects[i],"_warped_tlrc_dump.txt", sep = "")))[,4]
}
...
write.table(round(out_vec_med, 4), "preserved_warped_group_median.txt", row.names = F, col.names = F, quote = F)
