...
spec_func_linear_filt <- function(x)
{
    ...
    out <- c(usable_spec, round(linear_pval,4))
    return(out)
}
...
for (i in mod_list)
{
    ...
    if (length(as.matrix(which(spec_apply[15,] <= .05))) == 0)
    {
        spec_pval_filt = NA
    ...
    {
        spec_pval_filt <- as.matrix(which(spec_apply[15,] > .05)) ## filter by those that don't show significant linear trend
...
write.table(output, paste("modules.",Cond,".",Subj,"_powerexp_filtered",sep=""), row.names=F,col.names=F,quote=F)
