...
#subjects = ["CLFR", "TRCO"]
#subjects = ["LDMW", "MNGO"]
subjects = ["PIGL","SNNW","FLTM","EEPA","DNLN","CRFO","ANMS","MRZM","MRVV","MRMK","MRMC","MRAG","MYTP","LRVN","ANGO"]

for ss in subjects:
    os.chdir(os.environ["state"]+"/"+ss+"/corrTRIM_BLUR/another/")
    print os.getcwd()
    """
    Get the community (module) IDs for conditions 1 and 3
    """
    f3 = glob("cleanTS.3.*justcomm")
    f1 = glob(os.environ["state"]+"/"+ss+"/corrTRIM_BLUR/cleanTS.3.*justcomm")
    comf3 = open(''.join(f3))
    comf1 = open(''.join(f1))
    """
    ...
    outf = open("preserved_"+ss+"_another.txt","w")
    outf.write(pres_out)
    outf.close()
