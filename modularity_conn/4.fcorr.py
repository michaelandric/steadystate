...
class BCT_Corr:
    ...
    def deconvolve(self):
        ...
        print os.system("3dDeconvolve -input "+self.options.sourceD+"blur."+self.options.cc+"."+self.options.subj+".steadystate.TRIM+orig. -polort A -num_stimts 6 \
                        ...
                        -rout -bucket "+self.options.outD+"corr."+self.options.cc+"."+self.options.subj+".steadystate -errts "+self.options.outD+"cleanTS.masked.Cond"+self.options.cc+"."+self.options.subj)

    def maskdump(self):
        print "The out directory is: "+self.options.outD
        os.chdir(self.options.outD)
        print "Working in :"+os.getcwd()
        print os.system("3dmaskdump -mask "+self.options.dumpmask+" -noijk "+self.options.outD+"cleanTS.masked.Cond"+self.options.cc+"."+self.options.subj+"+orig.BRIK  > "+self.options.outD+"cleanTS."+self.options.cc+"."+self.options.subj+"_graymask_dump")
    ...
    def callR(self):
        ...
        print subprocess.call("R CMD BATCH --vanilla "+os.environ["state"]+"/codebase/modularity_conn/bct_fcorr.R", shell=True)
